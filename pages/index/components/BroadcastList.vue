<template>
  <div class="h-full shadow-md ring-1 ring-inset ring-gray-200 rounded-lg overflow-hidden transition duration-150 ease-out hover:ease-in hover:shadow-2xl">
    ...
    <div class="p-4 divide-y divide-y divide-dashed divide-gray-200">
      <div class="p-2 rounded cursor-pointer from-red-100 to-white hover:bg-gradient-to-b" v-for="item in listData" :key="item.broadcastId" @click="jumpTo(item.broadcastId)">
        ...
        <div class="leading-relaxed text-sm line-clamp-2 text-ellipsis text-justify mb-1">{{item.content}}</div>
