...
const {getProjectList, getNotificationList, jumpPath, initPageParams} = appStoreInfo;
...
const getList = async (projectId) => {
  getNotificationList({projectId, pageNum: currentPage.value, pageSize: pageSize.value}).then(rows => {
    list.value = rows || [];
  })
}

const goDetail = (item) => {
  jumpPath(`/notice/${item.notificationId}`)
}
</script>
